...
my $check = CheckTemplateDir->new( path_tiny_obj    => Path::Tiny->new("$FindBin::RealBin/data"),
                                 , suffix => '.tpl'
                                 );
...
$check = CheckTemplateDir->new( path_tiny_obj    => Path::Tiny->new('A/Non/Existing/Dir'),
                              , suffix => '.tpl'
                              );
...
$check = CheckTemplateDir->new( path_tiny_obj    => Path::Tiny->new("$FindBin::RealBin/data"),
                              , suffix => '.ABSENT'
                              );
...
warnings_are { $check->check_directory } [], "No warnings issued when matching files found.";
